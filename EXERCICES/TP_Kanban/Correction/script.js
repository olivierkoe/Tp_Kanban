...
class Notes {
  constructor(title, content, status = "todo") {
    ...
    (this.note = content), (this.status = status);
  }
}
...
function initialiseApp(noteArray) {
  ...
  if (yourNotes.length !== 0) {
    noteArray.forEach((note) => {
      ...
      if (
        singleNote.status === "todo" ||
        singleNote.status === "verified" ||
        singleNote.status === "verified"
      ) {
...
buttonSelector.addEventListener("click", function (e) {
  ...
  let newUserNote = new Notes(
    inputTitleSelector.value,
    inputNoteContentSelector.value
  );
...
function trashDrop(event) {
  ...
  console.log(
    `you drop an element is id is ${event.dataTransfer.getData("text")}`
  );
